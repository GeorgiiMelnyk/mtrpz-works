...
const convertMarkdown = (markdown) => {
    ...
    for (let i = 0; i < splitted.length - 1; i+= 2) {
        ...
        splitted[i] = splitParagraphs(splitted[i], tags)
    }
    splitted[splitted.length - 1] = replace(splitted[splitted.length - 1], tags)
    splitted[splitted.length - 1] = splitParagraphs(splitted[splitted.length - 1], tags)

    markdown = htmlTags.paragraph[0] + joinPreformatted(splitted, tags) + htmlTags.paragraph[1]
    return markdown
}
